...
1. **Cobertura parcial de erros nos VO`s financeiros**
   - `SocialBenefit.copyWith` lança `unwrap()` em caso de `FamilyMemberId` inválido, mas não há teste cobrindo o cenário (arquivo `packages/social/social-care/value-objects/SocialBenefit.valueObject.ts`).
...
1. **Adicionar regressões RED**
   - `packages/social/social-care/tests/regression/social-benefits.red.spec.ts` cobrindo:
