...
## Evolução do domínio
- Valorizações do domínio descritas em `../refactor/05-domain-purity-refactor.md` foram aplicadas: VO `Diagnosis` agora injeta `Timestamp`, o `SocialBenefitsCollection` concentra regras de listas de benefícios e `FamilyMemberId` garante UUID v7 válido.
...
- `../refactor/05-domain-purity-refactor.md`: registra a obtenção de pureza do domínio, abstração de dependências e padronização dos VOs.
- `../refactor/06-legacy-drop-and-bun-aliases.md`: documenta a exclusão do legacy, adoção de Bun + aliases e estabilização da surface pública.
...
- Testes de VOs permanecem verdes; suites de entidades (`packages/social/social-care/tests/unit/entities/`) intencionalmente falham até que as implementações sejam concluídas, conforme anotado no relatório 06.
