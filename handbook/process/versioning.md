...
- Use `bun pm version <patch|minor|major>` para atualizar `package.json` na raiz **e** cada pacote tocado em `packages/**`. Atalhos: `bun run version:patch|minor|major|prerelease` (scripts/version.py).
...
- `scripts/version.py` — wrapper leve para `bun pm version` + lembrete de changelog/tag.
