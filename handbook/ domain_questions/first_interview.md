...
### 1. Quais domínios independentes já aparecem nos pacotes atuais (packages/shared, packages/social/social-care) e onde ainda existem “concepts anêmicos”?
...
1.  **`Social Care Context` (Core Domain):** Localizado em `packages/social/social-care/`, este é o nosso Domínio Principal. Ele contém a lógica de negócio complexa e única do Conecta Social. Sua linguagem é específica para o atendimento social (ex: `Patient`, `RightsViolationReport`, `SocialHealthSummary`).
...
* **No Novo Modelo (`packages/social/social-care`):** O design que construímos é o **oposto de anêmico**; é um **Rich Domain Model**. A lógica de negócio está encapsulada nos próprios objetos de domínio.
...
1.  **Time 1: `Social Care Team` (Core Domain)**
    * **Responsabilidade:** Dono exclusivo do `packages/social/social-care`.
