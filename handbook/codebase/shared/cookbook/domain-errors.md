...
## Cenário
O agregado `Patient` delega toda validação para erros de domínio ricos (`@conecta/domain-error`). Precisamos manter catálogo, atalhos e integração HTTP alinhados ao que está versionado em `packages/conecta-raros/social-care/domain/errors`.
...
```ts typescript
// packages/conecta-raros/social-care/domain/errors/Patient.error.ts
import {
  ErrorTaxonomy,
...
```ts typescript
// packages/conecta-raros/social-care/domain/entities/Patient.entity.ts
...
- `catalog` e `codePrefix` refletem exatamente os códigos presentes em `packages/conecta-raros/social-care/domain/errors`.
