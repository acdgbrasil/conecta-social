  ...
  makeDomainErrorFactory<SocialCareAppointmentErrorKind>({
    ...
    catalog: {
      DateInFuture: {
        ...
        template: () => "A data do atendimento não pode estar no futuro em relação ao contexto fornecido.",
      },
      MissingNarrative: {
        code: "SCA-002",
        http: 422,
        category: ErrorTaxonomy.DomainRuleViolation,
        template: () => "É necessário informar ao menos resumo ou plano de ação para o atendimento.",
      },
      SummaryTooLong: {
        code: "SCA-003",
        http: 422,
        category: ErrorTaxonomy.DomainRuleViolation,
        template: ({ limit }) => `O resumo do atendimento não pode exceder ${limit} caracteres.`,
      },
      ActionPlanTooLong: {
        code: "SCA-004",
        http: 422,
        category: ErrorTaxonomy.DomainRuleViolation,
        template: ({ limit }) => `O plano de ação não pode exceder ${limit} caracteres.`,
      },
    },
  });
