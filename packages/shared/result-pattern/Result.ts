...
export const ok = <T, E = never>(value: T): Result<T, E> => ({
  ...
  unwrapErr: () => {
    ...
    }).create("CalledUnwrapErrOnOk", {
      cause: "Called unwrapErr on an Ok result",
      now: new Date(),
      st: new Error().stack,
    });
  },
...
export const err = <E = DomainError>(error: E): Result<never, E> => ({
  ...
  unwrap: () => {
    ...
    }).create("CalledUnwrapOnErr", {
      cause: "Called unwrap on an Err result",
      now: new Date(),
      st: new Error().stack,
    });
  },
