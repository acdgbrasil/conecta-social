...
describe("Referral.entity", () => {
  const NOW = new Date();
  const YESTERDAY_TS = Timestamp.create({
  ...
  describe("Criação (Factory)", () => {
    ...
    test("deve FALHAR ao criar com data no futuro (regra REF-001)", () => {
      // Arrange
      const TOMORROW_TS = Timestamp.create({
        value: new Date(NOW.getTime() + 24 * 60 * 60 * 1000),
      }).unwrap();
  ...
  describe("Máquina de Estados (Transições)", () => {
    ...
    test("deve FALHAR ao tentar transição inválida (ex: COMPLETED -> CANCELLED)", () => {
      ...
      const completedReferral = Referral.create(createValidProps(), NOW)
        .unwrap()
        .complete()
        .unwrap();
      ...
      expect(result.unwrapErr().code).toBe(
        RE.InvalidStatusTransition("COMPLETED", "CANCELLED").code,
      ); // REF-003
      expect(result.unwrapErr().message).toContain(
        "Não é permitido mover o encaminhamento de COMPLETED para CANCELLED",
      );
    });

    test("deve FALHAR ao tentar transição inválida (ex: CANCELLED -> COMPLETED)", () => {
      // Arrange
      const cancelledReferral = Referral.create(createValidProps(), NOW)
        .unwrap()
        .cancel()
        .unwrap();
      ...
      expect(result.isErr).toBe(true);
      expect(result.unwrapErr().code).toBe(
        RE.InvalidStatusTransition("CANCELLED", "COMPLETED").code,
      ); // REF-003
      expect(result.unwrapErr().message).toContain(
        "Não é permitido mover o encaminhamento de CANCELLED para COMPLETED",
      );
    });
  });
});
