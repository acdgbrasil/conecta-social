// packages/social/social-care/tests/unit/entities/rights-violation-report.entity.spec.ts
...
describe("RightsViolationReport.entity", () => {
  ...
  const TWO_DAYS_AGO_TS = Timestamp.create({
    value: new Date(NOW.getTime() - 2 * 24 * 60 * 60 * 1000),
  }).unwrap();

  // Helper para props válidas
  const createValidProps = (
    overrides: Partial<RightsViolationReportProps> = {},
  ) => ({
    ...
    descriptionOfFact:
      "Criança encontrada sozinha em casa por longos períodos.",
  ...
  describe("Identidade da Entidade", () => {
    test("deve considerar duas instâncias iguais se seus IDs forem os mesmos (equals)", () => {
      ...
      const report1 = RightsViolationReport.create(
        createValidProps({ id, actionsTaken: "Ação 1" }),
        NOW,
      ).unwrap();
