...
import type { DiagnosisProps } from "./props/diagnosis.props";
...
export class Diagnosis {
  ...
  static create(
    ...
    if (props.date.isAfter(now))
      return err(DE.DateInFuture(props.date.toISOString(), now.toISOString()));

    const year = props.date.getFullYear();
    if (year < 0) return err(DE.DateBeforeYearZero(year));

    if (!props.description || props.description.trim().length === 0)
      return err(DE.DescriptionEmpty(props.description));
