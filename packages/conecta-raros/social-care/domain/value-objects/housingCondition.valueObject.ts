...
import type { HousingConditionProps } from "./props/housingCondition.props";
...
export class HousingCondition implements HousingConditionProps {
  ...
  static create(
    ...
    if (props.numberOfBathrooms > props.numberOfRooms)
      return err(HC.BathroomsExceedRooms());

    return ok(new HousingCondition(props));
  }

  copyWith(
    props: Partial<HousingConditionProps>,
  ): Result<HousingCondition, DomainError> {
    return HousingCondition.create({
      housingConditionType:
        props.housingConditionType ?? this.housingConditionType,
      ...
      isInGeographicRiskArea:
        props.isInGeographicRiskArea ?? this.isInGeographicRiskArea,
      isInSocialConflictArea:
        props.isInSocialConflictArea ?? this.isInSocialConflictArea,
      electricityAccess: props.electricityAccess ?? this.electricityAccess,
      sewerDisposalMethod:
        props.sewerDisposalMethod ?? this.sewerDisposalMethod,
      wasteCollectionType:
        props.wasteCollectionType ?? this.wasteCollectionType,
