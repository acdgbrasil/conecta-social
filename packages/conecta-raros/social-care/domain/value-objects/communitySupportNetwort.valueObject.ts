...
import type { CommunitySupportNetworkProps } from "./props/communitySupportNetwork.props";
...
export class CommunitySupportNetwork implements CommunitySupportNetworkProps {
  ...
  static create(
    props: CommunitySupportNetworkProps,
  ): Result<CommunitySupportNetwork, DomainError> {
  ...
  copyWith(
    props: Partial<CommunitySupportNetworkProps>,
  ): Result<CommunitySupportNetwork, DomainError> {
    return CommunitySupportNetwork.create({
      hasSupportFromRelatives:
        props.hasSupportFromRelatives ?? this.hasSupportFromRelatives,
      hasSupportFromNeighbors:
        props.hasSupportFromNeighbors ?? this.hasSupportFromNeighbors,
      familyConflicts: props.familyConflicts ?? this.familyConflicts,
      patientParticipatesInGroups:
        props.patientParticipatesInGroups ?? this.patientParticipatesInGroups,
      familyParticipatesInGroups:
        props.familyParticipatesInGroups ?? this.familyParticipatesInGroups,
      patientHasAccessToLeisure:
        props.patientHasAccessToLeisure ?? this.patientHasAccessToLeisure,
      facesDiscriminationInCommunity:
        props.facesDiscriminationInCommunity ??
        this.facesDiscriminationInCommunity,
    });
  }
}
