...
import { type ImutableList, ImutableListFactory } from "@conecta/fn";
...
import {
  createDtoIcdCode,
  createDtoPersonID,
} from "packages/conecta-raros/social-care/application/dto/patient.dto";
...
export class RegisterNewPatientUseCase {
  ...
  async execute(
    ...
    if (existsResult.unwrap() === true)
      return err(AppError.PersonIdAlreadyExists());
    ...
    const immutableDiagnosisList: ImutableList<Diagnosis> =
      ImutableListFactory.fromArray(diagnosisList);
    const registerNewPatient = Patient.createFromScratch(
      personID,
      immutableDiagnosisList,
    );
