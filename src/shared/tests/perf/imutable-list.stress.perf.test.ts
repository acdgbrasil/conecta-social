...
describe("Stress Perf — ImutableList (Full Coverage Load)", () => {
  for (const size of SIZES) {
    test(`Carga Total com N = ${size.toLocaleString()}`, () => {
      ...
      // eslint-disable-next-line no-console
      console.log(`[Query] count & isEmpty (O(1)): ${(tQ2 - tQ1).toFixed(4)}ms`);
      ...
      // eslint-disable-next-line no-console
      console.log(`[Search] List.has (Worst case): ${(tH2 - tH1).toFixed(2)}ms`);
      ...
      // eslint-disable-next-line no-console
      console.log(`[Check] hasDuplicates (False): ${(tHD2 - tHD1).toFixed(2)}ms`);
      ...
      // eslint-disable-next-line no-console
      console.log(`[Filter] List.filter (Even numbers): ${(tF2 - tF1).toFixed(2)}ms`);
      ...
      // eslint-disable-next-line no-console
      console.log(`TOTAL para ${size.toLocaleString()} itens: ${(endTotal - startTotal).toFixed(2)}ms`);
    });
  }
});