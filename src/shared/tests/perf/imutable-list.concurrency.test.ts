...
describePerf("Full Concurrency & Starvation Analysis (All Ops)", () => {
	const SIZE = 500_000;
	const baseList = List.from(Array.from({ length: SIZE }, (_, i) => i));

	// CENÁRIO 1: Integridade de todas as operações sob concorrência
	test(
		"Integrity Attack: Todas as operações rodando simultaneamente",
		async () => {
		...
		{ timeout: 30000 },
	); // Aumenta timeout para 30s devido à carga massiva
	...
	test("Battery Starvation: Bloqueio acumulado de todas as operações O(N)", async () => {
		...
		const stressList = List.from(
			Array.from({ length: STRESS_SIZE }, (_, i) => i % 1000),
		);
		...
		List.filter(stressList, (_x) => true); // Filter
		...
		const actualBeats = heartbeats.filter(
			(t) => t >= tStart - startMonitor && t <= tEnd - startMonitor,
		).length;

		// eslint-disable-next-line no-console
		console.log(
			`[Starvation] Tempo total da bateria: ${totalProcessing.toFixed(2)}ms`,
		);
		// eslint-disable-next-line no-console
		console.log(
			`[Starvation] Heartbeats durante a bateria (esperados ~${expectedBeats}): ${actualBeats}`,
		);
		...
		// eslint-disable-next-line no-console
		console.log(
			`[Starvation] Alerta: O Event Loop ficou paralisado por ${totalProcessing.toFixed(2)}ms.`,
		);
	});
});
